...
from fanpage_agent.v2.audit.auditor import AuditManager
from fanpage_agent.v2.core.bus import AgentBus
from fanpage_agent.v2.core.harness import AgentHarness
...
def create_pipeline(
    ...
    if enable_llm:
        try:
            from fanpage_agent.v2.adapters.llm_adapter import LLMAdapter as _RealLLM
...
def _run_restore(data_dir: str, backup_idx: int = 1) -> None:
    from fanpage_agent.v2.memory.performance import PerformanceMemory, BackupError
    memory = PerformanceMemory(Path(data_dir) / "memory.db")
    try:
