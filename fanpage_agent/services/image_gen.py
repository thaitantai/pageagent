...
class UrlImageService(ImageService):
    ...
    def generate(self, prompt: str, output_dir: str | Path | None = None) -> str:
        ...
        if not url.startswith("http"):
            raise ValueError(f"UrlImageService requires a valid HTTP(S) URL, got: {prompt[:100]}")
...
def build_image_service(settings: Settings) -> ImageService:
    """Create an ImageService from application Settings."""
    provider = settings.img_provider.lower()
    out = settings.artifacts_dir / "images"

    if provider == "mock":
        return MockImageService(output_dir=out)
    if provider == "openai":
    ...
    if provider == "url":
        return UrlImageService(output_dir=out)
