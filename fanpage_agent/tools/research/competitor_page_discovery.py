...
class FacebookPageClient(Protocol):
    """Protocol tối thiểu mà CompetitorPageDiscoveryTool cần."""
...
class CompetitorPageDiscoveryTool:
    ...
    def discover(
        ...
        if self._fb_client is None:
            logger.warning(
                "CompetitorPageDiscoveryTool: no fb_client provided, "
                "cannot fetch posts"
            )
            return [], []
    ...
    def _scan_text(
        ...
        Sử dụng logic tương tự OfferDiscoveryTool._scan_text() nhưng
