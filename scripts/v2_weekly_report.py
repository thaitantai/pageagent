#!/usr/bin/env python3
"""
V2 Weekly Analyst Report — reads memory.db + state.json + container status.
...
DB_PATH = PROJECT / "data" / "v2" / "memory.db"
STATE_PATH = PROJECT / "data" / "v2" / "state.json"
...
def container_status() -> str:
    ...
    try:
        out = subprocess.run(
            ["docker", "ps", "--filter", "name=fanpage-agent-v2",
             "--format", "{{.Status}}"],
